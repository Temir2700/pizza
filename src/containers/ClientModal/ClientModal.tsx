  ...
  return (
    <div className="backdrop">
      <div className="modal">
          ...
          {cartPizzas.map((oneCartPizza) => {
            return (
              <div key={oneCartPizza.pizza.id} className="oneCartPizza">
                <div>
                  <img
                    alt={oneCartPizza.pizza.id}
                    src={oneCartPizza.pizza.image}
                    className="oneCartPizza-img"
                  />
                </div>
                <div>
                  {oneCartPizza.pizza.title} x {oneCartPizza.amount}
                </div>
                <div>{oneCartPizza.pizza.price} KGS</div>
                <button
                  type="button"
                  className="modal-btn-remove"
                  onClick={() => onItemDelete(oneCartPizza.pizza.id)}
                ></button>
              </div>
            );
          })}
        ...
        <div>
          Delivery <strong>150</strong> KGS
        </div>
        {cartPizzas.length > 0 ? (
          <div>
            Total: <strong>{sum + 150} KGS</strong>
          </div>
        ) : (
          <div>
            Total: <strong>{sum} KGS</strong>
          </div>
        )}
        <div className="modal-btns">
          <button className="btn" onClick={() => dispatch(setShow(false))}>
            Cancel
          </button>
